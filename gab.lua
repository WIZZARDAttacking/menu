Citizen.CreateThread(function()
    ...
    local textures = {
        ...
        { name = "ListAdm", link = "https://raw.githubusercontent.com/scmrl/xithwid/refs/heads/main/AdminList.png", width = 310, height = 51 },
    }
    ...
    function DrawTextOutline(text, x, y, scale, font, outline, center, r, g, b)
        ...
        SetTextFont(1)
        if outline then SetTextOutline(outline) end
    ...
    function DrawAdmList()
        if ESP.listaadm and #admlist > 0 then
            local xPos = 0.2 + main.x
            local yPos = 0.25 + main.y
            
            -- Desenha a textura da lista de ADM
            DrawSprite("ListAdm", "ListAdm", xPos, yPos, 0.13, 0.035, 0, 19, 19, 19, 255)
            
            -- Desenha o título
            DrawTextColor("Administradores", xPos - 0.06, yPos - 0.014, false, 0.26, 11, false, 255, 255, 255, 255)
            
            -- Desenha cada administrador na lista
            local yPosition = yPos
            local yAdd = 0.035
            for i, text in ipairs(admlist) do
                yPosition = yPosition + yAdd
                DrawRect(xPos, yPosition, 0.13, 0.035, 12, 12, 12, 100)
                DrawTextColor(text, xPos - 0.06, yPosition - 0.014, false, 0.26, 11, false, 240, 240, 240, 240)
            end
        end
    end
    ...
    function RunESP()
        ...
        -- ESP Linhas
        if ESP.esp_linhas then
            local c00rds = GetEntityCoords(PlayerPedId())
            for an in EnumeratePeds() do
                local cC = GetEntityCoords(an)
                local me = an ~= PlayerPedId()
                if IsEntityOnScreen(an) and me then
                    local cD = GetDistanceBetweenCoords(GetFinalRenderedCamCoord(), cC.x, cC.y, cC.z, true) * (1.6 - 0.05)
                    if cD < 400 then
        ...
        if ESP.espesq then
            for ped in EnumeratePeds() do
                ...
                if d < 999 and not IsEntityDead(ped) and IsEntityOnScreen(ped) and ped ~= PlayerPedId() then
                    if HasEntityClearLosToEntity(PlayerPedId(), ped, 19) then
                        local r, g, b = 254, 208, 24
                        local bone1 = GetPedBoneCoordsF(ped, 31086)
                        local bone2 = GetPedBoneCoordsF(ped, 0x9995)
                        local bone3 = GetPedBoneCoordsF(ped, 0xE0FD)
                        local bone4 = GetPedBoneCoordsF(ped, 0x5C57)
                        local bone5 = GetPedBoneCoordsF(ped, 0x192A)
                        local bone6 = GetPedBoneCoordsF(ped, 0x3FCF)
                        local bone7 = GetPedBoneCoordsF(ped, 0xCC4D)
                        local bone8 = GetPedBoneCoordsF(ped, 0xB3FE)
                        local bone9 = GetPedBoneCoordsF(ped, 0x3779)
                        local bone10 = GetPedBoneCoordsF(ped, 0xB1C5)
                        local bone11 = GetPedBoneCoordsF(ped, 0xEEEB)
                        local bone12 = GetPedBoneCoordsF(ped, 0x49D9)
                        local bone13 = GetPedBoneCoordsF(ped, 0x9D4D)
                        local bone14 = GetPedBoneCoordsF(ped, 0x6E5C)
                        local bone15 = GetPedBoneCoordsF(ped, 0xDEAD)
                        
                        if bone1 and bone2 then DrawLine(bone1, bone2, r, g, b, 255) end
                        if bone2 and bone3 then DrawLine(bone2, bone3, r, g, b, 255) end
                        if bone4 and bone3 then DrawLine(bone4, bone3, r, g, b, 255) end
                        if bone5 and bone3 then DrawLine(bone5, bone3, r, g, b, 255) end
                        if bone6 and bone5 then DrawLine(bone6, bone5, r, g, b, 255) end
                        if bone7 and bone6 then DrawLine(bone7, bone6, r, g, b, 255) end
                        if bone8 and bone4 then DrawLine(bone8, bone4, r, g, b, 255) end
                        if bone8 and bone9 then DrawLine(bone8, bone9, r, g, b, 255) end
                        if bone2 and bone10 then DrawLine(bone2, bone10, r, g, b, 255) end
                        if bone10 and bone11 then DrawLine(bone10, bone11, r, g, b, 255) end
                        if bone11 and bone12 then DrawLine(bone11, bone12, r, g, b, 255) end
                        if bone2 and bone13 then DrawLine(bone2, bone13, r, g, b, 255) end
                        if bone13 and bone14 then DrawLine(bone13, bone14, r, g, b, 255) end
                        if bone14 and bone15 then DrawLine(bone14, bone15, r, g, b, 255) end
                    end
                end
            end
        end
        ...
        if ESP.esp_box then
            ...
            for _, targetPlayer in ipairs(GetActivePlayers()) do
                ...
                if targetPed ~= playerPed then
                    local width = 0.3
                    local height = 1.7
                    local offsetX, offsetY, offsetZ = 0.0, 0.0, -1.0
                    ...
                    if not IsEntityVisibleToScript(targetPed) then
                    ...
                    DrawLine(leftBottomBack.x, leftBottomBack.y, leftBottomBack.z, rightBottomBack.x, rightBottomBack.y, rightBottomBack.z, color[1], color[2], color[3], color[4])
            ...
            for _, player in ipairs(GetActivePlayers()) do
                ...
                if myped ~= -1 and myped ~= nil then
                    ...
                    if not IsEntityVisibleToScript(myped) and distance <= maxDistance then
                        if not IsEntityDead(myped) and IsEntityOnScreen(myped) then
    ...
    function DrawMenu()
        ...
        if (main.tab == "jogador") then
            if (main.subtab == "Jogador") then
                ...
                if (Button("Reviver", false, 0.459, 0.325)) then
                    ...
                    Notify("Sucesso", "keeper-sucess", "Revivido!", 255, 255, 255)
                end

                if (Button("Suicidio", false, 0.459, 0.365)) then
                    SetEntityHealth(PlayerPedId(), 0)
                    Notify("Sucesso", "keeper-sucess", "Suicidio!", 255, 255, 255)
                end

                if (Button("Limpar Ferimentos", false, 0.459, 0.405)) then
                    ClearPedBloodDamage(PlayerPedId())
                    Notify("Sucesso", "keeper-sucess", "Ferimentos Limpos!", 255, 255, 255)
                end

                if (Button("Teleport Marcado", false, 0.459, 0.445)) then
                    TPWaypoint()
                end
        ...
        elseif (main.tab == "jogadores") then
            if (main.subtab == "Players") then
                ...
                if (Button("Matar jogador", false, 0.459, 0.405)) then
                    Citizen.CreateThread(function()
                        if SelectedPlayer then
                            ...
                            ShootSingleBulletBetweenCoords(coords.x, coords.y, coords.z - 0.5, coords.x, coords.y, coords.z, 1000, true, GetHashKey('VEHICLE_WEAPON_TURRET_PATROLBOAT_50CAL'), PlayerPedId(), true, false, -1.0, true)
                            Notify("Sucesso", "keeper-sucess", "Player morto!", 255, 255, 255)
                        else
                            Notify("Aviso", "keeper-warn", "Selecione um jogador primeiro!", 255, 255, 255)
                        end
                    end)
                ...
                if (Button("Taser Player", false, 0.459, 0.615)) then
                    if SelectedPlayer then
                        ...
                        Notify("Sucesso", "keeper-sucess", "Taser aplicado!", 255, 255, 255)
                    else
                        Notify("Aviso", "keeper-warn", "Selecione um jogador primeiro!", 255, 255, 255)
                    end
                end
            ...
            elseif (main.subtab == "Visual") then
                ...
                if (CheckBox("ESP Nomes", false, 0.53, 0.315, ESP.esp_nome)) then
                    ESP.esp_nome = not ESP.esp_nome
                end

                if (CheckBox("ESP Linhas", false, 0.53, 0.348, ESP.esp_linhas)) then
                    ESP.esp_linhas = not ESP.esp_linhas
                end

                if (CheckBox("ESP Esqueletos", false, 0.53, 0.381, ESP.espesq)) then
                    ESP.espesq = not ESP.espesq
                end

                if (CheckBox("ESP Vida", false, 0.53, 0.415, ESP.vidabarra)) then
                    ESP.vidabarra = not ESP.vidabarra
                end

                if (CheckBox("ESP Box", false, 0.53, 0.448, ESP.esp_box)) then
                    ESP.esp_box = not ESP.esp_box
                end

                if (CheckBox("ESP Veiculos", false, 0.53, 0.481, ESP.esp_vehs)) then
                    ESP.esp_vehs = not ESP.esp_vehs
                end

                if (CheckBox("Lista ADM", false, 0.53, 0.515, ESP.listaadm)) then
                    ESP.listaadm = not ESP.listaadm
                end
            ...
            if (main.subtab == "Armas") then
                ...
                if (Button("Adicionar Componentes", false, 0.459, 0.615)) then
                    Citizen.CreateThread(function()
                        ...
                        local selectedWeapon = GetSelectedPedWeapon(playerPed)
                        if selectedWeapon ~= GetHashKey("WEAPON_UNARMED") then
                            GiveWeaponComponentToPed(playerPed, selectedWeapon, GetHashKey("COMPONENT_AT_AR_FLSH"))
                            GiveWeaponComponentToPed(playerPed, selectedWeapon, GetHashKey("COMPONENT_AT_SCOPE_MEDIUM"))
                            GiveWeaponComponentToPed(playerPed, selectedWeapon, GetHashKey("COMPONENT_AT_AR_AFGRIP"))
                ...
                if (Button("Adicionar Munição", false, 0.459, 0.655)) then
                    Citizen.CreateThread(function()
                        ...
                        if weapon ~= GetHashKey("WEAPON_UNARMED") then
                            SetPedAmmo(PlayerPedId(), weapon, math.ceil(Sliders["Municao"].Value))
                            Notify("Sucesso", "keeper-sucess", "Munição adicionada com sucesso!", 255, 255, 255)
                        else
                            Notify("Aviso", "keeper-warn", "Você não está segurando uma arma!", 255, 255, 255)
                        end
                    end)
                ...
                if (CheckBox("NoRecoil", false, 0.737, 0.641, norecoil)) then
                    ...
                    if norecoil then
                        local weaponsList = {
                            0xBFE256D4, 0x5EF9FEC4, 0xEFE7E2DF, 0x624FE830, 0x394F415C,
                            0x969C3D67, 0xBFEFFF6D, 0xC0A3098D, 0xFAD1F1C9, 0x84D6FAFD,
                            0x83BF0278, 0x61012683, 0x13532244, 0x78A97CD0, 0x2BE6766B,
                            0x0A3D4D34, 0x7FD62962
                        }
            ...
            if (Button("Deletar Veículo", false, 0.459, 0.445)) then
                Citizen.CreateThread(function()
                    ...
                    if DoesEntityExist(vehicle) then
                        ...
                        Notify("Sucesso", "keeper-sucess", "Veiculo deletado com sucesso!", 255, 255, 255)
                    else
                        Notify("Aviso", "keeper-warn", "Entre em um veiculo primeiro!", 255, 255, 255)
                    end
                end)
            ...
            if (Button("Spawnar Veiculo", false, 0.459, 0.615)) then
                if SelectedVehicle then
                    Citizen.CreateThread(function()
                        ...
                        SetEntityAsMissionEntity(carGen, true, true)
                        PlaceObjectOnGroundProperly(carGen)
                        Wait(1000)
        ...
        elseif (main.tab == "Veiculos") then
            ...
            if (Button("Explodir Veiculo", false, 0.459, 0.445)) then
                Citizen.CreateThread(function()
                    if SelectedVehicles then
                        local veh = SelectedVehicles
                        if DoesEntityExist(veh) and veh ~= GetVehiclePedIsIn(PlayerPedId()) then
                            NetworkExplodeVehicle(veh, 1, false, 0)
                            Notify("Sucesso", "keeper-sucess", "Veiculo explodido!", 255, 255, 255)
                        else
                            Notify("Aviso", "keeper-warn", "Selecione um veículo válido!", 255, 255, 255)
                        end
                    else
                        Notify("Aviso", "keeper-warn", "Selecione um veículo primeiro!", 255, 255, 255)
                    end
        ...
        elseif (main.tab == "misc") then
            if (main.subtab == "Config") then
                ...
                if (Button("Desinjetar", false, 0.459, 0.325)) then
                    main.Loop = false
                    Notify("Sucesso", "keeper-sucess", "Menu desinjetado!", 255, 255, 255)
                end

                if (Button("Resetar Binds", false, 0.459, 0.365)) then
                    Notify("Sucesso", "keeper-sucess", "Binds resetados!", 255, 255, 255)
                end
    ...
    function FunctionsLoop()
        if SSoco then
            ...
            if hit == 1 then
                local EnType = GetEntityType(entityHit)
                if EnType == 2 then
            ...
            Citizen.CreateThread(function()
                ...
                if driver ~= 0 and driver ~= PlayerPedId() then
                    ...
                    if vehicle and vehicle ~= 0 then
                        SetVehicleExclusiveDriver(vehicle, driver, 1)
                    end
                end
            end)
        end
    ...
    -- ===================================================
    -- LOOP PRINCIPAL - MODIFICADO PARA EXIBIR ADM LIST MESMO COM MENU FECHADO
    -- ===================================================
    CreateThread(function()
        while main.Loop do
            Wait(0)
            ...
            -- A lista de ADM agora é desenhada independente do menu estar aberto
            DrawAdmList()
